    ...
    public class NewsHideController : BaseAuthedController
    {
        ...
        public ActionResult Index()
        {
            try
            {
                ...
                model.ListNew = _newsbussiness.GetListNewStatusByFilter(userId, 0, 0, 0, 0, -1, string.Empty, string.Empty, 0, -1, model.pageIndex, model.pageSize, Convert.ToInt32(CMS.Helper.NewsStatus.IsDelete), ref total);
                model.Total = total;
                ...
                minPrice, double maxPrice, string from, string to, int pageIndex, int pageSize)
        {
            try
            {
                int userId = Convert.ToInt32(Session["SS-USERID"]);
                int total = 0;
                var listNews = _newsbussiness.GetListNewStatusByFilter(userId, cateId, districtId, newTypeId, siteId, backdate, from, to, minPrice, maxPrice, pageIndex, pageSize, Convert.ToInt32(CMS.Helper.NewsStatus.IsDelete), ref total);
